...
describe('ChessPanel — board position', () => {
  it('shows the initial FEN when no node is selected', () => {
    ...
    expect(screen.getByTestId('chessboard').dataset.fen).toBe(fenAfterMoves('root'));
  });

  it('shows the FEN for the selected node (span-2: e4 e5 Nf3)', () => {
    render(<ChessPanel selectedNodeId="span-2" />);
    expect(screen.getByTestId('chessboard').dataset.fen).toBe(fenAfterMoves('span-2'));
  });

  it('shows the FEN for a single-move node (e4)', () => {
    render(<ChessPanel selectedNodeId="e4" />);
    expect(screen.getByTestId('chessboard').dataset.fen).toBe(fenAfterMoves('e4'));
  });
...
describe('ChessPanel — move list in French (fr)', () => {
  ...
  it('shows bishop moves as F (Fou), not A (Alfil)', () => {
    ...
    render(<ChessPanel selectedNodeId="span-4" />);
